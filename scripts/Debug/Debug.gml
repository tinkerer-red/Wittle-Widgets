...
#region jsDoc
/// @func   test_progress_factorial_1_to_10k()
/// @desc   Computes factorial independently for every integer from 1 to 10,000.
///         For each n, computes n! from scratch.
///         Designed to exercise the progress logger under increasing workload.
#endregion
function test_progress_factorial_1_to_10k() {

var _total_numbers = 10_000_000;
var _slot_index = 0;
var _bar_width = 30;

// Outer loop: each number gets its own factorial
for (var _number = 1; _number <= _total_numbers; _number += 1) {

	// Compute factorial of _number from scratch
	var _factorial_value = 1;

	for (var _i = 1; _i <= _number; _i += 1) {
		_factorial_value *= _i;

		// Once it overflows to inf, further multiplies are pointless
		// but still deterministic; break to avoid wasted cycles
		if (is_infinity(_factorial_value)) {
			break;
		}
	}

	// Touch the value so the loop cannot be optimized away
	if (_factorial_value < 0) {
		_factorial_value = 1;
	}

	// Report progress based on completed numbers
	log_progress(
		_number,
		_total_numbers,
		"<prefix>",
...
test_progress_factorial_1_to_10k()